...
Wc_c="`wc -c < 'zm.c'`"
test 18384 -eq "$Wc_c" ||
        echo 'zm.c: original size 18384, current size' "$Wc_c"
fi
# ============= zmr.c ==============
...
X               strcpy(p, s); return p;
X       }
X       fprintf(stderr, "No memory for mystrsave!\n");
X       exit(1);
}
X
...
X
#ifdef XARGSFILE
X       vfile("npats=%d *patts=%s", npats, *patts);
X       if (npats == 1 && !strcmp(XARGSFILE, *patts)) {
X               in = fopen(XARGSFILE, "r");
X               if (!in) {
X                       printf(stderr, "Can't open / control file!\n");
X                       exit(2);
X               }
X               for (npats=0,argv=patts=xargv; npats<XARGSMAX; ++npats,++argv) {
X                       if (fgets(txbuf, 1024, in) <= 0)
X                               break;
X                       uncrlf(txbuf);
X                       *argv = mystrsave(txbuf);
X               }
X               fclose(in);
X       }
#endif
...
X       bytcnt = -1;
X       if (Nozmodem) {
X               printf("Start your YMODEM receive. ");  fflush(stdout);
X       }
X       for (n=0; n<argc; ++n) {
X               Totsecs = 0;
...
X       "Usage: sz [-2+abcdefklLnNuvwyY] [-] file ...",
X       "\t     zcommand [-2Cev] COMMAND",
X       "\t     zcommandi [-2Cev] COMMAND",
X       "\t     sb [-2adfkuv] [-] file ...",
X       "\t     sx [-2akuv] [-] file",
...
X       fprintf(stderr,"\tCopyright 1991 Omen Technology INC All Rights Reserved\n");
X       exit(3);
}
X
/*
...
Wc_c="`wc -c < 'sz.c'`"
test 35578 -eq "$Wc_c" ||
        echo 'sz.c: original size 35578, current size' "$Wc_c"
fi
# ============= rz.c ==============
...
Wc_c="`wc -c < 'rz.c'`"
test 27957 -eq "$Wc_c" ||
        echo 'rz.c: original size 27957, current size' "$Wc_c"
fi
# ============= crctab.c ==============
...
Wc_c="`wc -c < 'crc.c'`"
test 8796 -eq "$Wc_c" ||
        echo 'crc.c: original size 8796, current size' "$Wc_c"
fi
# ============= rbsb.c ==============
...
X
X               tty.c_cflag &= ~(PARENB|HUPCL);         /* Disable parity */
...
X       if (Tty < 0) {
X               perror("/dev/tty");  exit(2);
X       }
X       Ttystream = fdopen(Tty, "w");
X       setbuf(Ttystream, xXbuf);               
}
X
flushmoc()
{
...
Wc_c="`wc -c < 'rbsb.c'`"
test 9549 -eq "$Wc_c" ||
        echo 'rbsb.c: original size 9549, current size' "$Wc_c"
fi
# ============= minirb.c ==============
